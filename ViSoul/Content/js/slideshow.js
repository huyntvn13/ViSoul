...
(function($){
	$.fn.pexetoSlideshow=function(options){
		...
		function showNavigation(){
			...
			$('#slider-navigation-container').animate({bottom:46}, 500);
		...
		function hideNavigation(){
			...
			$('#slider-navigation-container').animate({bottom:0}, 500);
		...
		function resizeImage(image) {
			var theWindow = $(window), $bg = image.img, aspectRatio = image.width
					/ image.height;
			...
			if ((theWindow.width() / theWindow.height()) < aspectRatio) {
				...
				var newRatio = theWindow.height() / image.height;
				$bg.css( {
							height : '100%',
							width : 'auto',
							minWidth : 0,
							minHeight : 0,
							marginLeft : -(image.width * newRatio - theWindow
									.width()) / 2
						});
			} else {
				root.css( {
